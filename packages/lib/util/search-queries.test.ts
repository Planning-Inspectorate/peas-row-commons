...
describe('createWhereClause', () => {
	describe('single options', () => {
		...
		it('should create where clause for single query', () => {
			...
			assert.deepStrictEqual(result, { AND: [{ OR: [{ OR: [{ name: { contains: 'foo' } }] }] }] });
		});

		it('should create where clause for multiple queries', () => {
			const result = createWhereClause(['foo', 'bar'], options);
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ OR: [{ name: { contains: 'foo' } }] }] }, { OR: [{ OR: [{ name: { contains: 'bar' } }] }] }]
			});
		});
		...
		it('should use the searchType if provided', () => {
			...
			const result = createWhereClause(['foo', 'bar'], opts);
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ OR: [{ name: { startsWith: 'foo' } }] }] }, { OR: [{ OR: [{ name: { startsWith: 'bar' } }] }] }]
			});
		});
	});
				...
				AND: [
					{
						OR: [
							...
							{ Profile: { OR: [{ email: { startsWith: 'baz' } }] } }
						]
					},
					{
						OR: [
							{ User: { OR: [{ first: { contains: 'buzz' } }, { last: { contains: 'buzz' } }] } },
							{ Profile: { OR: [{ email: { startsWith: 'buzz' } }] } }
						]
					}
				]
			});
	...
	describe('constraints', () => {
		it('should include constraints in each field condition (single field)', () => {
			...
			const result = createWhereClause(['foo'], options);
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ AND: [{ OR: [{ comment: { contains: 'foo' } }] }, { commentRedacted: { equals: null } }] }] }]
			});
		});
		...
		it('should include constraints with parent', () => {
			...
			assert.deepStrictEqual(result, {
				AND: [
					{ OR: [{ AND: [{ Rep: { OR: [{ text: { contains: 'bar' } }] } }, { status: { equals: 'PUBLISHED' } }] }] }
				]
			});
		});
	});
	...
	describe('edge cases', () => {
		...
		it('should allow non-string query values (e.g. numbers) - passed through', () => {
			...
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ OR: [{ id: { equals: 123 } }] }] }, { OR: [{ OR: [{ id: { equals: 456 } }] }] }]
			});
		});
		it('should keep empty string queries if provided explicitly', () => {
			const options = [{ fields: ['name'], searchType: 'contains' }];
			const result = createWhereClause([''], options);
			assert.deepStrictEqual(result, { AND: [{ OR: [{ OR: [{ name: { contains: '' } }] }] }] });
		});
	});
	...
	describe('1-many relations and relationConstraints', () => {
		it('should use "some" operator for 1-many relations when isList is true', () => {
			...
			const result = createWhereClause(['foo'], options);
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ Contacts: { some: { OR: [{ name: { contains: 'foo' } }] } } }] }]
			});
		});
		...
		it('should apply relationConstraints inside the parent object for 1-1 relations', () => {
			...
			const result = createWhereClause(['foo'], options);
			assert.deepStrictEqual(result, {
				AND: [{ OR: [{ Status: { AND: [{ OR: [{ name: { contains: 'foo' } }] }, { isActive: true }] } }] }]
			});
		});
						...
						OR: [
							{
								Contacts: {
									some: {
										AND: [{ OR: [{ name: { contains: 'foo' } }] }, { contactTypeId: 'applicant-appellant' }]
									}
								}
							}
						]
