 ...
 */
export function casesToViewModel(cases: CaseListFields[], groupMembers: EntraGroupMembers) {
	const caseOfficers = groupMembers.caseOfficers || [];
	const inspectors = groupMembers.inspectors || [];

	return cases.map((caseRow) => {
		const caseOfficerId = caseRow.CaseOfficer?.idpUserId;
		const mappedCaseOfficer = caseOfficers.find((user) => user.id === caseOfficerId);

		const mappedInspectors = caseRow.Inspectors.map((insp) => {
			const inspectorId = insp.Inspector?.idpUserId;
			const mappedInspector = inspectors.find((user) => user.id === inspectorId);
