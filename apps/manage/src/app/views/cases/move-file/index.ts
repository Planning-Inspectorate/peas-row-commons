...
export function createRoutes(service: ManageService): IRouter {
	...
	const [handleMoveSelection, viewMoveFiles, handleDuplicateErrors] = createMiddlewares(service);
	...
	router.route('/').get(handleDuplicateErrors, asyncHandler(viewMoveFiles)).post(asyncHandler(handleMoveSelection));
...
function createMiddlewares(service: ManageService) {
	return [buildHandleMoveSelection(), buildViewMoveFiles(service), buildHandleDuplicateFileNamesMiddleware()];
}
