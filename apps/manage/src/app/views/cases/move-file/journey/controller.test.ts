...
describe('Move Files Controller', () => {
	...
	describe('buildSaveController', () => {
		...
		it('should record an audit entry for each file moved', async () => {
			...
			assert.strictEqual(recordedAudits[0].metadata.newFolderName, 'Destination Folder');
			...
			assert.strictEqual(recordedAudits[1].metadata.newFolderName, 'Destination Folder');
		});
		...
		it('should use fallback dash when source folder name is missing', async () => {
			...
			assert.strictEqual(recordedAudits[0].metadata.newFolderName, 'Target');
		});
		...
		it('should use fallback dash when destination folder name is missing', async () => {
			...
			assert.strictEqual(recordedAudits.length, 1);
			assert.strictEqual(recordedAudits[0].metadata.newFolderName, '-');
