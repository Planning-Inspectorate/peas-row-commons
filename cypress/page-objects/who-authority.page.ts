class WhoAuthorityPage {
	isPageDisplayed(fullValidation = true): void {
		...
		cy.contains('.govuk-body', 'To add a new authority to the list or to add multiple authorities to your case contact')
			.should('exist')
			.and('be.visible');
		...
		cy.get('[data-cy="button-save-and-continue"]').should('exist').and('be.visible').and('contain.text', 'Continue');
