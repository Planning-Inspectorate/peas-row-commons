	...
	isPageDisplayed(mode: ApplicantDetailsPageMode, state: ApplicantDetailsState, fullValidation = true): void {
		runPageValidation(
			...
			() => {
				...
				cy.contains('a.govuk-button', 'Add details')
					.should('exist')
					.and('be.visible')
					.and('have.attr', 'href')
					.and(
						'match',
						mode === 'createCase'
							? /\/cases\/create-a-case\/questions\/applicant-details\/add\/[0-9a-f-]+\/applicant-name$/
							: /\/cases\/[0-9a-f-]+\/applicant-details\/add\/[0-9a-f-]+\/applicant-name$/
					);
